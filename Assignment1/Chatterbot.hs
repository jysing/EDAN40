...
substitute _ [] _ = []
substitute y (x:xs) ys
  | y == x = ys ++ substitute y xs ys
  | otherwise = x : substitute y xs ys
...
transformationApply wc f x (p, s) = mmap (substitute wc s . f) $ match wc p x


-- Applying a list of patterns until one succeeds
transformationsApply :: Eq a => a -> ([a] -> [a]) -> [([a], [a])] -> [a] -> Maybe [a]
transformationsApply wc f p x = foldr1 orElse $ map (transformationApply wc f x) p 
